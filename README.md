...
This `es_augmentenv` branch incorperates the evolutionary strategy and augmented environments from [*Reinforcement Learning for Improving Agent Design*](https://arxiv.org/abs/1810.03779) [Ha 2019]
...
- Several [example agents](/agents) that learn (or evolve) via a function specified by the reward predictor
...
Then run the following to install the rl-teacher code into your conda environment:
    ...
    pip install -e agents/es-augmentenv
...
#Evolving Agents
To evove agents using augmented morphologies, you must both set the agent flag `-a` to `es_augment` and pass an 
augmentable environment inheriting from [AugmentMujocoXMLEnv](agents/es-augmentenv/envs/generate_mujoco_xml_env.py) 
```angular2html
