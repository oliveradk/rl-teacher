...
def make_model(game, make_env_inner):
  # can be extended in the future.
  model = Model(game, make_env_inner)
  return model
...
class Model:
  ...
  def __init__(self, game, make_env_inner=None):
    ...
    self.make_env_inner = make_env_inner
  ...
  def make_env(self, seed=-1, render_mode=False):
    if self.make_env_inner:
      self.env = self.make_env_inner()
      return

    self.render_mode = render_mode
    self.env = AugmentHopper()
    if seed >= 0:
      self.env.seed(seed)

  def augment_env(self):
    env = self.env if hasattr(self.env, "augment_env") else self.env.env
    env.augment_env(self.body_param)
